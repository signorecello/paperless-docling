...
async function findTargetTag() {
    try {
        ...
        const tag = tagsResponse.data.results.find(tag => tag.name === TAG_NAME);
        if (!tag) {
...
async function getDocumentsWithTag() {
    try {
        ...
        const documentsResponse = await axios.get(`${PAPERLESS_API_URL}/documents/`, {
            headers: {
                'Authorization': PAPERLESS_AUTH,
                'Accept': 'application/json; version=2'
            },
...
async function downloadDocument(documentId) {
    try {
        const response = await axios.get(`${PAPERLESS_API_URL}/documents/${documentId}/download/`, {
            headers: {
                'Authorization': PAPERLESS_AUTH,
                'Accept': 'application/json; version=2'
            },
...
async function removeTagFromDocument(documentId) {
    try {
        ...
        const response = await axios.get(`${PAPERLESS_API_URL}/documents/${documentId}/`, {
            headers: {
                'Authorization': PAPERLESS_AUTH,
                'Accept': 'application/json; version=2'
            }
        ...
        const updatedTags = currentTags.filter(tagId => tagId !== targetTagId);
...
async function processDocument(document) {
    ...
    try {
        ...
        processingPath = await downloadDocument(documentId);

        // Create temporary directory for docling output
        await fs.mkdir(outputDir, { recursive: true });

        // Build the docling command with all configurable parameters
        const doclingCommand = `docling --pipeline ${DOCLING_PIPELINE} --vlm-model ${DOCLING_MODEL} --device ${DOCLING_DEVICE} --num-threads ${DOCLING_THREADS} --output ${escapePath(outputDir)} --pdf-backend ${DOCLING_PDF_BACKEND} ${DOCLING_EXTRA_ARGS} ${escapePath(processingPath)}`;
        ...
        // Clean up
        if (processingPath) {
            await fs.unlink(processingPath).catch(err => console.error(`Error removing temporary file: ${err}`));
        }
        await fs.rm(outputDir, { recursive: true, force: true }).catch(err => console.error(`Error removing output directory: ${err}`));
...
app.listen(PORT, () => {
    ...
    console.log(`Checking for new documents every ${CHECK_INTERVAL/1000} seconds`);
}); 
